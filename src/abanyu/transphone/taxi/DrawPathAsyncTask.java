...
public class DrawPathAsyncTask extends AsyncTask<Void, Void, String>{
  ...
  public DrawPathAsyncTask(Context taxiMapContext, String stringUrl){
	context = taxiMapContext;
  ...
  private void drawPath(String jsonizedStringUrl){
	try{
	  ...
	  for(int i = 0; i < list.size()-1; i++){ 
		...
		TaxiMap.line = TaxiMap.map.addPolyline( new PolylineOptions().add(new LatLng(src.latitude, src.longitude),   //marker 1
				                                                          new LatLng(dest.latitude, dest.longitude)) //marker 2
				                                .width(2) 
				                                .color(Color.BLUE)
				                                .geodesic(true));                        
	  }		
